const app = {
    ...
    switchTab: function (tabName) {
        ...
        if (tabName === 'mindmap') {
            ...
            if (this.state.currentModule.slide) {
                pdfPath = this.state.currentModule.slide;
            } else {
                const details = this.getMindmapDetails();
                if (details) {
                    pdfPath = `Video/Chuong_${details.chapter}_Tiet_${details.lesson}/slide.pdf`;
                }
            }

            if (pdfPath && !slideFrame.src.includes(pdfPath)) {
